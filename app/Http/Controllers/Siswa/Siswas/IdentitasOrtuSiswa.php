...
class IdentitasOrtuSiswa extends Controller
{
    public function index($ortu)
    {
        $siswa = Auth::user();
        ...
        if ($data){
            ...
            if ($foto){
                $req['foto_'.$ortu] =UrlFileHelper::get($siswa,'thumb_'.$foto,'siswa');
            } else {
    ...
    public function update(SiswaOrtuAdminUpdate $request, $ortu)
    {
        $siswa = Auth::id();
